...
\begin{center}
  ...
  \textbf{Linked P7 pre-registration DOI:}
  [to be inserted upon Zenodo confirmation]\\
...
The field-arrest model (Lawson, 2026 ---
see linked theory paper) proposes that the
causal origin is not genetic but epigenetic:
the domestication coherence gradient field
($N_{\text{domestic}}$) suppresses the maternal
HPA axis, which alters the intrauterine coherence
gradient environment, which establishes
H3K27me3 enrichment at NCC migration driver
loci via EZH2 activity, which arrests NCC
migration at the juvenile-proximate developmental
stage.
...
\begin{tabular}{p{2.2cm}p{10.5cm}}
\toprule
\textbf{Gene} & \textbf{Rationale for Selection as Negative Control} \\
\midrule
\textit{GAPDH} &
...
as P7). Gestational age confirmed by
...
(same standard as P7). Any wild boar sample
with $\geq 5\%$ admixture excluded.
This controls for the possibility that
wild boar samples with partial domestic
ancestry show partial H3K27me3 enrichment
that contaminates the wild boar baseline.
...
[Zenodo DOI pending] \\[0.3em]
P10 &
Breed Depth vs.\ Feralization Rate &
[Zenodo DOI pending] \\[0.3em]
P11 &
Tazemetostat in Pregnant Sow:
F1 Morphology Shift &
[Zenodo DOI pending ---
requires P9 confirmation first] \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.3em}
\textbf{Theory paper from which P9 was derived:}

Lawson, E.R. (2026).
\textit{Domestication as Field-Specified
Developmental Arrest: A Unified Causal
Mechanism for the Domestication Syndrome
Derived from Attractor Geometry.}
OrganismCore / Zenodo. [DOI pending]
