#!/usr/bin/env python3
"""
KRvK Perfect-Play Analyzer
=============================
One tool, three stages, one shared codebase. Direct adaptation of
kqvk_analyzer.py: same three-stage pipeline, rook geometry instead of queen
geometry, and move parsing generalized to any piece letter (see the note on
apply_move/child_turn below) rather than hardcoded to 'Q'.

  classify   Runs over the WHOLE landscape database (from krvk_solver's
             --batch or --full-dag), classifies every multi-way tie as
             TRIVIAL_MATE / EXPLAINED / INDEPENDENT / INDEPENDENT_UNVERIFIED.
             Writes all_classifications.csv and
             independent_findings_deduped.csv.
...
=====================================================================
USAGE -- the full pipeline, in order, exactly as you'd type it later:
=====================================================================

    # 1. Classify every multi-way tie in the whole KRvK database
    python3 krvk_analyzer.py classify krvk_perfect_play.db --out-dir krvk_tie_analysis

    # 2. Compile families vs genuinely unique findings, and auto-render a
    #    full verified tree for every one of them in the same command
    python3 krvk_analyzer.py families krvk_tie_analysis/independent_findings_deduped.csv \\
        --out-dir krvk_families --render-trees krvk_perfect_play.db

    # 3. (optional) Drill into one specific position by hand
    python3 krvk_analyzer.py tree krvk_perfect_play.db \\
        --position "WK:a4 WQ:e4 BK:h4" --turn W --out tree.txt \\
        --classifications krvk_tie_analysis/all_classifications.csv

Note on that "WQ:" in the example above: it's not a typo and not a queen.
The C++ solver's internal position-string format always labels White's
second piece "WQ:" regardless of what piece it actually is (a pragmatic
choice made to avoid renaming that field across the whole engine), so a
KRvK position string still reads "WK:.. WQ:.. BK:.." with WQ: holding the
rook's square. The move notation itself DOES use the real piece letter
('R' for a rook move, e.g. "Rb8") -- only the position-string label stays
"WQ:" for backward compatibility with the existing file format and tools.
"""
...
def canonical_form(triple):
    """Smallest image of (wk,wr,bk) coords under the 8-element D4 group."""
    best = None
    for T in TRANSFORMS:
...
# apply_move/child_turn are piece-letter-agnostic (checked via .isupper()),
# NOT hardcoded to 'Q' or 'R' specifically. This matters here: KRvK move
# notation uses the real piece letter -- a rook move looks like "Rb8", not
# "Qb8" -- so a hardcoded check for 'Q' (as kqvk_analyzer.py's predecessor
# scripts originally had) would silently fail to recognize every single
# rook move, misparsing the resulting position. Checking "is this letter
# uppercase" instead correctly recognizes ANY white-piece letter, so this
# same logic would also carry over unchanged to a bishop, knight, etc.
...
def is_symmetric_triple(triple_a, triple_b):
    """triple_* = (wk, wr, bk) as squares. Returns the transform index (0 =
...
# Direct adaptation of is_attacked_by_queen: file/rank blocking checks only,
# no diagonal branch -- a rook simply doesn't threaten off-file/off-rank
# squares. Cross-validated against the C++ RookRules::attacks it mirrors
# (320 randomized geometry checks, zero mismatches) before being used here.
def is_attacked_by_rook(pos, rp, wk, wr):
    if pos == rp:
        return False
    pf, pr = pos
    rf, rr = rp
    if pf == rf:
        ...
        for r in range(lo + 1, hi):
            if (pf, r) == wk or (pf, r) == wr:
                return False
        return True
    if pr == rr:
        lo, hi = sorted([pf, rf])
        for f in range(lo + 1, hi):
            if (f, pr) == wk or (f, pr) == wr:
                return False
        return True
    return False
...
def is_checkmate_state(state):
    ...
    try:
        p = parse_position_coords(pos_str)
        wk, wr, bk = p['WK'], p['WQ'], p['BK']
    except (KeyError, ValueError, IndexError):
        return False
    if not is_attacked_by_rook(bk, wr, wk, wr):
        return False
    for m in king_moves(bk):
        if is_attacked_by_rook(m, wr, wk, wr):
            continue
...
def black_legal_move_count(wk, wr, bk):
    cnt = 0
    for m in king_moves(bk):
        if is_attacked_by_rook(m, wr, wk, wr):
            continue
...
# Defined before `families` so cmd_families can call it directly.
# ============================================================================
...
# ============================================================================
# STAGE 2: families, with the --render-trees integration so "compile the
# list" and "look at one" don't require a separate invocation.
# ============================================================================
...
def main():
    ...
    p3.add_argument('--out-dir', default='krvk_trees')
