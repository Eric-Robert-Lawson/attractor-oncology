...
class ValidationAnalyzer:
    ...
    def analyze_mismatches(self):
        ...
        # Critical mismatches
        critical = [m for m in self.mismatches if m['landscape']['M'] < m['DTZ']]
        if critical:
            print(f"\n⚠ CRITICAL MISMATCHES (M < DTZ): {len(critical)}")
            print("=" * 120)
        ...
        # Positive mismatches
        positive = [m for m in self.mismatches if m['landscape']['M'] > m['DTZ']]
        if positive:
            print(f"\n⚠ SUBOPTIMAL MISMATCHES (M > DTZ): {len(positive)}")
            print("=" * 120)
    ...
    def _print_mismatch_detail(self, mismatch):
        ...
        print(f"  Landscape M: {landscape['M']} (delta: {landscape['M'] - dtz:+d})")
    ...
    def write_mismatches(self, output_file):
        ...
        with open(output_file, 'w') as f:
            f.write("Position,DTZ,M,Delta,Status,BestMove,FullGameLine,NumMoves,Plies,WhiteMoves,BlackMoves,NodesEval,CompTime,BN_Trajectory\n")
            ...
            for mismatch in sorted(self.mismatches, key=lambda x: abs(x['delta']), reverse=True):
                ...
                total_plies = landscape['total_plies']
                ...
                f.write(f'"{pos}",{dtz},{m},{delta:+d},{status},"{best_move}","{game_line}",{num_moves},{total_plies},{white_moves},{black_moves},{nodes_eval},{comp_time:.3f},"{bn_traj}"\n')
    ...
    def write_matches(self, output_file):
        ...
        with open(output_file, 'w') as f:
            f.write("Position,DTZ,M,BestMove,FullGameLine,NumMoves,Plies,WhiteMoves,BlackMoves,NodesEval,CompTime,BN_Trajectory\n")
            ...
            for match in self.matches:
                ...
                f.write(f'"{pos}",{dtz},{m},"{best_move}","{game_line}",{num_moves},{total_plies},{white_moves},{black_moves},{nodes_eval},{comp_time:.3f},"{bn_traj}"\n')
