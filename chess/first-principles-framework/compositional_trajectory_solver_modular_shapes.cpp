...
public:
    ...
    // Keys loaded from a PREVIOUS run's exported database, trusted as
    // already-proven facts and seeded into `classified` before the
    // fixed-point loop even starts -- see seed_from_preloaded's comment for
    // why this is sound, unlike the old single-attacker engine's REMOVED
    // depth-budget-gated shortcut. Tracked separately so export can skip
    // re-writing rows that are already persisted on disk, letting the
    ...
    // Seeds `classified` with facts loaded from a PREVIOUS run's exported
    // database, called before classify()'s fixed-point loop begins -- not
    // mid-recursion, and with no depth-budget concept involved at all. This
    // is a fundamentally different situation from the old single-attacker
    // engine's removed shortcut, which checked the database DURING a
    // depth-limited recursive search and accepted a cached value as long as
    // it numerically fit inside whatever budget remained for that specific
    // call -- a gate that measured "does the ANSWER fit the budget", not
    // "was this answer actually, honestly verified", and was confirmed to
    // both produce wrong answers and hang indefinitely when reconstructed
    // and tested directly.
    //
    // Here, a loaded value is trusted for exactly the same reason a
    // checkmate is trusted as distance 0: the ONLY way any row ever entered
    // the exported file is after this same fixed-point process, in an
    // earlier run, already fully proved it -- justified by facts that were
    // themselves already fully justified, bottoming out in checkmate or
    // proven-insufficient-material terminals, with no partial-budget gap
    // anywhere in the chain. A position's true value doesn't depend on
    // which run or which path discovered it; reusing an honestly-proven
    // fact is always sound, unconditionally -- that's the whole point of
    // building this as a fixed-point classifier instead of a depth-limited
    ...
    void discover(const vector<GeneralState>& roots) {
        ...
        while (!queue.empty()) {
            ...
            if ((long long)nodes.size() - last_reported >= 1000000) {
                ...
                cout << "  [discovery progress] " << nodes.size() << " positions found so far, "
                     ...
                     << elapsed << "s (" << sealed_count << " already-proven positions sealed, "
                     << "not re-expanded)\n" << flush;
            }
            uint64_t key = queue.front(); queue.pop_front();
            if (nodes.count(key)) continue;
            // SEALING: a position already proven (loaded from a resumed
            // database) never needs its children expanded here -- nothing
            // will ever need them again. Its value alone (already in
            // `classified` via seed_from_preloaded) is everything any other
            // position referencing it as a child could possibly need,
            // recomputed cheaply on demand by child_black_and_mobility if
            // needed at all. This is the actual memory reduction: without
            // it, a resumed run still rebuilds the ENTIRE reachable graph
            // in memory before classify() gets to skip anything -- measured
            // directly: a prior version of this reported "Discovery: 402688
        ...
        if (sealed_count) cout << "  Sealed " << sealed_count << " already-proven positions during discovery "
                                << "(not re-expanded -- this is what keeps a resumed run's memory bounded "
                                << "by NEW work, not the full graph size)\n";
    }

    // Returns (is_black_to_move, own_mobility) for a child key that might
    // NOT be present in `nodes` at all -- specifically, a "sealed" position
    // (already known from a preloaded/resumed database, whose children were
    // deliberately never expanded during discovery, to avoid re-holding an
    // already-fully-solved subtree in memory). Falls back to recomputing
    // both directly from the position itself when that happens -- cheap,
    // since this is one position's own move generation, not a subtree
    // expansion, and it's only ever needed for positions that are already
    // fully proven, never for anything still being verified.
    pair<bool, int> child_black_and_mobility(uint64_t ckey) const {
        auto it = nodes.find(ckey);
        if (it != nodes.end()) {
            bool is_black = (it->second.flags & 4) == 0;
            int mobility = is_black ? (int)it->second.children.size() : 0;
            return {is_black, mobility};
        }
        GeneralState st = unpack_general_state(ckey);
        bool is_black = (st.to_move == 'B');
        int mobility = is_black ? (int)generate_legal_moves(st).size() : 0;
        return {is_black, mobility};
    }
            ...
            for (auto& [key, node] : nodes) {
                ...
                if (white_to_move) {
                    ...
                    for (auto& ckey : node.children) {
                        ...
                        auto [c_is_black, c_mobility] = child_black_and_mobility(ckey);
                        int contribution = (c_is_black ? c_mobility : 0) + cit->second.escape_cum;
                    ...
                    for (auto& ckey : node.children) {
                        ...
                        auto [c_is_black, c_mobility] = child_black_and_mobility(ckey);
                        int contribution = (c_is_black ? c_mobility : 0) + cres.escape_cum;
...
// Parses the SPACE-separated format GeneralState::str() itself produces
// ("Q:h8 K:a1 k:h1") -- genuinely different from parse_general_position
// above, which parses the COMMA-separated seed-file format ("K:a1,Q:h8,k:h1").
// Confirmed as a real, distinct bug during testing: using the comma parser
// on a database row silently failed every single line (0 positions loaded
// from a real 345,412-row file), because the whole unsplit string doesn't
// match that parser's expected shape and it correctly rejects it rather
// than guessing -- the fix is using the right parser for the right format,
// not loosening either one's validation.
inline optional<GeneralState> parse_general_state_str(const string& s) {
    GeneralState st;
    st.to_move = 'W';
    stringstream ss(s);
    string token;
    while (ss >> token) {
        if (token.size() < 3 || token[1] != ':') return nullopt;
        char letter = token[0];
        string sq_str = token.substr(2);
        if (sq_str.length() != 2) return nullopt;
        Position sq;
...
// Loads a previously-exported general-engine database, for resuming a run
// rather than starting from scratch. Returns (packed key -> (distance,
// escape_cum)) for every row -- see seed_from_preloaded's comment for why
// these are safe to trust unconditionally, not just re-checked heuristics.
// Returns an empty map (not an error) if the file doesn't exist yet -- a
// first-ever run for a given material is exactly that, a normal case, not
// a failure.
inline unordered_map<uint64_t, pair<int,int>> load_general_db_for_resume(const string& path) {
    unordered_map<uint64_t, pair<int,int>> result;
    ifstream file(path);
    ...
    while (getline(file, line)) {
        ...
        auto st = parse_general_state_str(parts[0]);
        if (!st) continue;
        st->to_move = parts[1].empty() ? 'W' : parts[1][0];
        int distance, escape_cum;
        try {
            distance = stoi(parts[3]);
            escape_cum = stoi(parts[4]);
        } catch (...) { continue; }
        result[pack_general_state(*st)] = {distance, escape_cum};
    }
    if (!result.empty()) cout << "  Loaded " << result.size() << " previously-solved positions from " << path << "\n";
    return result;
}
...
// solved directly.
//
// Appends rather than truncates, and skips any key already present in
// rc.preloaded_keys -- those rows are already on disk from an earlier run,
// re-writing them would just duplicate every line. The header is written
// only when the file doesn't already exist, so a resumed run's output
// stays one clean, valid CSV rather than accumulating a header mid-file.
inline void export_general_classification(RetrogradeClassifier& rc, const string& db_file) {
    bool file_already_exists = ifstream(db_file).good();
    ofstream out(db_file, ios::app);
    if (!file_already_exists) out << "Position|Turn|BestMove|Distance|EscapeCum|TiedMoves\n";
    long long written = 0, skipped_preloaded = 0;
    for (auto& [key, node] : rc.nodes) {
        if (rc.preloaded_keys.count(key)) { skipped_preloaded++; continue; }
        auto res = rc.get(key);
    ...
    out.close();
    cout << "Wrote " << written << " newly-classified positions to " << db_file;
    if (skipped_preloaded) cout << " (" << skipped_preloaded << " already-persisted positions skipped, not duplicated)";
    cout << "\n";
}
...
inline void run_general_sweep(const vector<GeneralState>& roots, const string& db_file,
                               ...
                               const vector<string>& preload_from_files = {}) {
    cout << "\n" << string(80, '=') << "\n";
    ...
    // Resume from a previous run's output, if one exists (use --fresh to
    // start clean instead -- that flag already deletes db_file before this
    // function is ever called, so an empty load here is the normal,
    ...
    // Additional, separately-solved materials' own databases, seeded the
    // same way and for the same reason -- see --preload-from's own help
    // text and seed_from_preloaded's comment for why this is sound
    // regardless of which run or which material originally proved a fact.
    // Additive to the same-material resume above, not a replacement for it:
    // both can be in play in the same run.
    for (auto& path : preload_from_files) {
        auto cross_preloaded = load_general_db_for_resume(path);
        long long wins_added = 0;
        bool mismatch_found = false;
        for (auto& [key, val] : cross_preloaded) {
            auto existing = rc.classified.find(key);
            if (existing == rc.classified.end()) {
                wins_added++;
            } else if (existing->second.distance != val.first || existing->second.escape_cum != val.second) {
                cerr << "FATAL: --preload-from " << path << " disagrees with an already-known fact "
                     << "for a shared position (key " << key << "): existing distance="
                     << existing->second.distance << " escape=" << existing->second.escape_cum
                     << ", this file says distance=" << val.first << " escape=" << val.second
                     << ". A genuinely proven fact never depends on provenance -- two sources "
                     << "disagreeing means at least one is wrong (wrong material fed in, a packing "
                     << "collision, or a real bug in one of the two runs that produced them), and "
                     << "this must be investigated before trusting either file further, not "
                     << "silently overwritten.\n";
                mismatch_found = true;
            }
        }
        if (mismatch_found) { return; }
        if (!cross_preloaded.empty()) rc.seed_from_preloaded(cross_preloaded);
        auto cross_preloaded_draws = load_general_draws_for_resume(path);
        long long draws_added = 0;
        for (auto& key : cross_preloaded_draws) {
            if (rc.proven_draws.find(key) == rc.proven_draws.end()) draws_added++;
        }
        if (!cross_preloaded_draws.empty()) rc.seed_draws_from_preloaded(cross_preloaded_draws);
        cout << "[preload-from] " << path << ": " << cross_preloaded.size() << " proven wins, "
             << cross_preloaded_draws.size() << " proven draws loaded ("
             << wins_added << " wins / " << draws_added << " draws newly added, "
             << "the rest already known from db_file's own resume above)\n";
    }
    ...
    long long already_classified_before = (long long)rc.classified.size();
    rc.classify();
    ...
    if (already_classified_before > 0) {
        cout << " (" << already_classified_before << " of those came pre-proven from the resumed database, "
...
int main(int argc, char* argv[]) {
    ...
    vector<string> preload_from_files;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (arg == "--preload-from" && i + 1 < argc) { preload_from_files.push_back(argv[++i]); }
    ...
    if (!unrecognized.empty()) {
        ...
        cerr << "\nKnown flags: --debug/-d, --batch/-b, --full-dag/-g, "
                ...
                "--preload-from <file> (repeatable, PIECE_GENERAL only) -- seed this run with "
                "already-proven facts from another material's own, separately-completed .db file. "
                "Sound whenever this run's own tree can reduce (via capture or promotion) into "
                "that other material's position space: a proven fact's truth never depends on "
                "which run discovered it, only on being honestly bottomed out in terminal "
                "positions (see seed_from_preloaded's own comment). Every position this run's "
                "discovery reaches that's already present in a preloaded file is trusted "
                "immediately and never re-expanded.\n";
        return 1;
    }
    ...
    run_general_sweep(roots, db_file, max_nodes_arg, preload_from_files);
#else
