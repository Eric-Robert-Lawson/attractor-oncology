...
**Implementation, three checkpointed passes in sequence, all over the same database:**
1. `find_shape_origins` — a single O(n) scan marking which positions have a forced-in parent, then flagging every tie and every position without one as an origin. Checkpoints to `<db_path>.origin_checkpoint.pkl`.
2. `resolve_reachable_shapes` — the main pass, in increasing distance order (topological, no recursion needed — a position's distance is always strictly greater than any of its children's), building each position's full set of reachable (shape id, escape count) pairs, using genuine set semantics (an earlier draft used a dict keyed by shape id and silently discarded one of two valid escape-count variants for the same mate depending on write order — confirmed and fixed). Checkpoints to `--checkpoint-path-landscape`, default `<db_path>.landscape_checkpoint.pkl`.
3. `summarize_shapes` — filters the second pass's output down to origins only, producing the final catalog.
