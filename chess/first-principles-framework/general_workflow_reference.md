...
**Phase 2 — full-landscape retrograde-basin shapes** (over the *entire* database, tied or not):
- This is not "canonicalize each position on its own" — an earlier version of this section did exactly that, and it was replaced after measuring it wasn't answering the actual question: it only ever removes ordinary 2-to-8-fold board-symmetry redundancy, so on a real landscape it barely reduces the position count at all (measured directly on KQvK's own database: 345,040 decision-bearing positions down to only 43,195 "shapes", a ratio matching plain D4 symmetry almost exactly, not anything meaningful about the landscape's actual structure).
- What it computes instead: every position's forced (non-tied) play is walked forward until it hits a terminal — either a checkmate, or a genuine multi-way tie acting as a boundary where the position's own future stops being a single, forced flow. That terminal, canonicalized by board symmetry, **is** the shape. Every position whose forced chain resolves to the identical terminal belongs to the identical shape, regardless of how far from that terminal it sits or what the intervening moves look like. This is a materially different, and much more meaningful, reduction: measured on the same KQvK database, this drops 345,404 positions to just 6,676 distinct shapes — roughly **51.7x**, not ~8x. Implemented with path compression (the same technique union-find structures use for the same reason), so a position's shape is only ever fully resolved once, however many other positions' chains pass through it.
- Produces `shapes.csv` — one row per genuinely distinct shape, with its recurrence count and one example position. `--full-position-map` additionally writes the full `Position,Turn -> ShapeId` mapping for every position (a genuine second pass, reusing the same shape registry so ids stay consistent with the main catalog — real size at scale, off by default).
- `--skip-full-landscape` — run Phase 1 only.
- Unlike Phase 1, there is currently no "already complete, skip and reload" check for this phase specifically — it always reruns fully when invoked. If switching to this version of the tool with an existing `--out-dir` from before the Phase 2 replacement, delete the old `decision_shapes.csv` / `position_to_shape.csv` first (superseded, not overwritten, since the output filename changed) and confirm no stale `<db_path>.landscape_checkpoint.pkl` remains from the earlier mechanism — its saved format won't match what the current code expects.
...
| `material_reduction/shapes.csv` | The full-landscape catalog: every distinct retrograde-basin shape (grouped by shared terminal, not per-position symmetry), with recurrence counts |
...
- **`reduce`'s Phase 2 was replaced, not just tuned.** The earlier version canonicalized each position independently and mostly just recovered ordinary board symmetry (~8x on KQvK, barely reducing anything meaningful). It's been replaced with a retrograde-basin grouping — every position sharing a forced-play destination counts as one shape — measured at ~51.7x on the identical database. Output file renamed `decision_shapes.csv` → `shapes.csv` as part of this; the two are not compatible, and switching to the current version against an existing `--out-dir` should include deleting the old file and any stale `<db_path>.landscape_checkpoint.pkl` first (see §1 Step 6).
