...
**What re-tuning actually costs, precisely, not just "cheap" or "expensive": redoing Phase 1 and Phase 2, but not the origin scan — unless you use `--skip-phase1-and-landscape` instead (see immediately below).** Once `shapes.csv` exists, re-reaching the `--cluster-mates`/`--trace-shape-graph-segmented` code path with `--force-rerun-phase1` also redoes Phase 1 (tie-branch reduction) and Phase 2's full landscape scan (`resolve_reachable_shapes`), not just the clustering step. That's real, non-trivial cost on a large material. The one genuine saving under `--force-rerun-phase1` itself: the origin scan (`find_shape_origins`) stays cached in `<db_path>.origins_cache.pkl` and is reused automatically — `--force-rerun-phase1` does not also force this unless `--force-rerun-origins` is passed separately.

**`--skip-phase1-and-landscape` — the real fix for the gap this section used to describe as open.** Confirmed directly, not theoretical: an interruption during the cluster pre-pass or write pass (which only start *after* Phase 2 has already finished successfully) left `--force-rerun-phase1` as the only way back in — but that flag forces the *entire* Phase 2 landscape scan to redo from scratch too, since that phase's own checkpoint is deleted the moment it first succeeds. At KBNvK scale (tens of millions of positions), that's a real, substantial, entirely avoidable cost for something that was already correct and complete.

`--skip-phase1-and-landscape` skips Phase 1 and Phase 2 entirely and goes straight to the trace/cluster step, reconstructing exactly what it needs from already-written, trusted output: `shape_registry` directly from `shapes.csv`'s own rows (a cheap CSV read, not a re-scan) and `origins`/`parent_counts` from the existing origins cache (already fast — that's what the cache is for). Requires both `shapes.csv` and `<db_path>.origins_cache.pkl` to already exist from a prior successful run — fails loudly, not silently, if either is missing, since the whole premise of this flag is "these are already correct, trust them directly," not "compute them if absent." Also validates the origins cache against the *current* database's own length and mtime before trusting it, the same safety check `--force-rerun-phase1`'s own cache-loading path already uses.

```bash
python3 general_analyzer.py reduce material_analysis/independent_findings_deduped.csv material_perfect_play.db --out-dir material_reduction --trace-shape-graph-segmented --cluster-mates --cluster-jaccard-threshold 0.5 --max-open-files 200 --skip-phase1-and-landscape
```

**Verified directly against the full path, not just assumed equivalent**: ran a complete, uninterrupted `reduce` on real KQvK data, then separately ran `--skip-phase1-and-landscape` against a fresh output directory seeded with only that run's own `shapes.csv` — output was **byte-for-byte identical** to the full path's own output, all 3,601,748 node-shape rows and all 412,152 edges, checked directly, not sampled.

**When this flag is *not* the right choice**: if the underlying `findings_csv` or database genuinely changed since `shapes.csv` was last written, this flag would happily reconstruct from stale, no-longer-correct data — `--force-rerun-phase1` (paying its real cost) or a fresh Phase 1/2 run is the correct choice there, not this shortcut.
...
**`--slow-origin-seconds` (default 1.0) — names the specific origin behind a slow progress-print interval, instead of leaving it to be inferred from the gap.** Built directly from a real, live observation, not spun up speculatively: on an actual KBNvK run, `[cluster pre-pass]` progress-print intervals (fixed ~10s apart) varied by roughly 200x — one interval advanced 221 origins, the very next advanced exactly 1 — with no way to tell which origin caused it or why. This prints a direct warning the instant any single origin's own exploration takes at least this many seconds, naming the position and (for `--cluster-mates`) exactly how many distinct mates it touched, with that origin's own O(k²) pairing cost shown alongside it. Confirmed directly, not just theorized, on real KQvK data: the slowest single origin observed (0.34s) touched 28 distinct mates — directly implicating the O(k²) pairing cost already flagged as this pre-pass's main scaling risk, rather than leaving it an open question. Re-verified with the same abrupt-kill rigor as everything else here: enabling this diagnostic and re-running the kill/resume test produced the same byte-for-byte-identical result against the baseline — the added timing/printing doesn't touch correctness or checkpoint state.
...
`findings_csv` (positional) · `db_path` (positional) · `--out-dir DIR` (default `general_reduction`) · `--force-rerun-phase1` · `--skip-phase1-and-landscape` (new — bypasses Phase 1/2 entirely using already-written `shapes.csv` + origins cache, see §1 Step 6 for when this is and isn't the right choice) · `--force-rerun-origins` · `--retry-skipped SKIPPED_CSV` · `--max-seconds-per-finding N` (default 60) · `--checkpoint-path FILE` · `--checkpoint-every-findings N` (default 200) · `--slow-finding-checkpoint-seconds N` (default 5.0) · `--checkpoint-path-landscape FILE` · `--checkpoint-every-seconds-landscape N` (default 60 — now shared by the full-landscape scan, `--cluster-mates`, and `--trace-shape-graph-segmented`, see §1 Step 6) · `--skip-full-landscape` · `--full-position-map` · `--trace-shape-graph` · `--trace-shape-ids ID1,ID2,...` (shared by both trace flags below) · `--trace-shape-graph-segmented` (memory-bounded, writes segmented SQLite directly, see §1 Step 6) · `--trace-shape-segmented-out-dir DIR` (default `<out-dir>/shape_graph_segmented`) · `--trace-shape-routing-path FILE` (default `<out-dir>/shape_graph_routing.sqlite`) · `--max-open-files N` (default 100 — must comfortably exceed the material's distinct-mate, or distinct-group with `--cluster-mates`, count) · `--cluster-mates` (new — pre-pass that groups tightly-related mates into shared files, see §1 Step 6) · `--cluster-jaccard-threshold N` (default 0.5 — the value actually verified end-to-end, see §1 Step 6) · `--cluster-max-group-size N` (default 10) · `--slow-origin-seconds N` (default 1.0 — names the specific origin behind a slow progress interval, see §1 Step 6)
