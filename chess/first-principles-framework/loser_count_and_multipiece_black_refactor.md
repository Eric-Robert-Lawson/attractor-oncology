...
6. **Two things built separately from this refactor will need revisiting once it lands, not just the classifier itself.** Both assume the current binary (White-wins-or-draw) outcome model structurally, not incidentally:
   ...
   - **The shapes/origin methodology** (a separate design document, `shape_based_compression_and_scoped_generation.md`): a "shape" is currently defined as `(mate, escape count)`, where "mate" implicitly means "White mates Black" — the only outcome the current engine can produce. Once Black can also mate White, a shape's identity needs to additionally capture *which side* gets mated, and the origin-finding logic (`find_shape_origins`) needs re-examining for whether "nothing forces play into this position" still means the same thing once a position's defender might be actively trying to force their own win rather than only ever delaying.

---

## One-paragraph summary, for a very fast re-entry

The escape-count metric was refined to count only the legal moves of the specific piece the losing side's perfect play actually moves at each decision point (not all of that side's pieces), renamed "loser node count" to reflect that the losing side need not be Black once Black can have real material — this refinement is a verified no-op on everything already computed, since a bare king has no other piece to disambiguate. Extending the engine so Black can have real pieces (and can win) is a genuine, scoped refactor — not a simple remap — because two specific places in `discover()` currently hard-code "only Black can ever be mated" (the checkmate-terminal flag, and the White-only material-sufficiency check), and the whole classification model needs to expand from binary (White-wins-or-draw) to a genuine three-outcome space (White wins / Black wins / draw). The underlying retrograde fixed-point methodology itself is not in question — it's sound, and doesn't need to be replaced, only extended. One piece of concrete groundwork is already banked, separately from this refactor: the engine's packed-state bit budget was confirmed (while raising White's own piece limit to 5) to hold exactly 5 non-king pieces total regardless of how they're eventually distributed between sides, so this refactor's design work won't need to revisit the piece-count ceiling itself — see 3e. Two other things built independently of this refactor will need revisiting once it lands, not redesigning from scratch but not free either: the new `--preload-from` cross-material seeding flag and its safety check, and the shapes/origin methodology's own definition of "mate" — both currently assume the binary outcome model structurally — see item 6 in Part 4.
