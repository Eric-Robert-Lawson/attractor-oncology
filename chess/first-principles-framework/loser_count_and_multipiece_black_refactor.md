...
6. **Two things built separately from this refactor will need revisiting once it lands, not just the classifier itself.** Both assume the current binary (White-wins-or-draw) outcome model structurally, not incidentally:
   - **`--preload-from`** (a new `general_solver` flag, seeding a run with an already-solved, separately-completed *material's* database — see the workflow reference, §1 Step 2): its loader (`load_general_db_for_resume`) returns a bare `(distance, escape)` pair per position, with no way to represent "this position is actually a forced loss for whoever's sweep is consuming it." Once a preloaded material's own results can include Black wins, both the loader's return type and the mismatch-safety check built alongside it (which currently only compares distance and escape) need a third field for which side the result actually favors.
   - **The shapes/origin methodology** (a separate design document, `shape_based_compression_and_scoped_generation.md`, now in its final, corrected form as of this writing): a "shape" is `(mate, cumulative escape count)`, where "mate" is a canonical `(position, turn)` pair. A more precise version of what needs revisiting than an earlier draft of this document stated:
     - **A shape's identity itself does not need an added field for which side won.** A given `(position, turn)` has exactly one true value — it cannot simultaneously be "White just delivered mate" and "Black just delivered mate" — so the canonical mate identity already implicitly encodes the outcome once the position is fully solved. Nothing new needs to be invented here.
     - **What actually breaks is narrower and more specific: `resolve_reachable_shapes`'s own terminal-detection check.** It currently treats `not entry.tied` (no further moves) as unconditionally meaning "checkmate, White won" — correct today only because `db` currently ever stores White-winning positions at all, so an entry with no tied moves has nothing else it could be. Once a ternary-outcome database can contain Black-winning positions too, this same check would still correctly mean "checkmate" (a won position with no further moves is still necessarily a mate), but the surrounding code would need to know *which side* delivered it — most naturally by reading whatever outcome field Part 4 item 1's own redesign of `RetroResult` ends up adding, not by inventing a separate mechanism specific to shapes.
     - **Genuinely reassuring, not just neutral, and worth stating plainly: the rest of the shapes/origin machinery is already structurally agnostic to any of this.** `find_shape_origins` and the tie-processing inside `resolve_reachable_shapes` both iterate over `entry.tied` uniformly, with no branching on whose turn it is or which side a tie's candidates belong to — a tie is an intersection, a position with no forced-in parent is a fresh start, regardless of which side is doing the choosing. That means the origin *concept* itself should survive this refactor intact; only the terminal/mate-detection step needs revisiting, not the origin-finding step around it.
     - **Also already compatible, and this is worth confirming explicitly rather than assuming: escape count itself needs no changes on the shapes side.** `resolve_reachable_shapes` never computes an escape contribution — it only ever reads and accumulates whatever value (`bn`) the C++ engine already placed in `entry.tied`. Once the engine itself correctly implements the piece-scoped "loser node count" from Part 2, the shapes/origin machinery in Python inherits that correctness automatically, with no changes of its own required to how that value gets read or summed.

---

## One-paragraph summary, for a very fast re-entry

The escape-count metric was refined to count only the legal moves of the specific piece the losing side's perfect play actually moves at each decision point (not all of that side's pieces), renamed "loser node count" to reflect that the losing side need not be Black once Black can have real material — this refinement is a verified no-op on everything already computed, since a bare king has no other piece to disambiguate. Extending the engine so Black can have real pieces (and can win) is a genuine, scoped refactor — not a simple remap — because two specific places in `discover()` currently hard-code "only Black can ever be mated" (the checkmate-terminal flag, and the White-only material-sufficiency check), and the whole classification model needs to expand from binary (White-wins-or-draw) to a genuine three-outcome space (White wins / Black wins / draw). The underlying retrograde fixed-point methodology itself is not in question — it's sound, and doesn't need to be replaced, only extended. One piece of concrete groundwork is already banked, separately from this refactor: the engine's packed-state bit budget was confirmed (while raising White's own piece limit to 5) to hold exactly 5 non-king pieces total regardless of how they're eventually distributed between sides, so this refactor's design work won't need to revisit the piece-count ceiling itself — see 3e. Two other things built independently of this refactor will need revisiting once it lands, though narrower than it might first appear: `--preload-from`'s safety check needs a third field for which side a result favors, and the shapes/origin methodology's terminal-detection step (currently assuming "no further moves" always means White won) needs to read whichever outcome field the ternary redesign adds — but a shape's own identity needs no new field (a solved `(position, turn)` already implies who won), and the origin-finding logic itself is already turn-agnostic and needs no changes at all — see item 6 in Part 4 for the precise breakdown of what does and doesn't need touching.
